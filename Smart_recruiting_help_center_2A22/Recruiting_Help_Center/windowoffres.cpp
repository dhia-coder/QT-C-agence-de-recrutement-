...
void WindowOffres::o_stat(){
    QSqlQuery q,q1;
    q.prepare("select count(ID_ENTREPRISE) , niveau from offre group by niveau;");
    q1.prepare("select count(ID_ENTREPRISE) from offre;");
...
{
    QSqlQuery q;
    q.prepare("select id from societe where NOM='"+ui->o_comboBox_2->currentText()+"';");
    q.exec();
    q.next();
...
QString id=ui->o_comboBox_2->currentText();
...
{
    offre o(ui->o_comboBox_7->currentText());
    QSqlQuery query;
...
{
    offre o;
    o.set_id(ui->o_comboBox_7->currentText());
