    ...
    , ui(new Ui::WindowEntreprises)
{
    ...
    // Name should contain only alphabetic characters (letters and spaces)
    QRegExp nameRx("^[a-zA-Z\\s]+$");
    QRegExpValidator* nameValidator = new QRegExpValidator(nameRx, this);
    ui->lineEdit_2->setValidator(nameValidator); // Assuming this is for the 'name' field
    ui->lineEdit_3->setValidator(nameValidator); // Assuming this is for another name-related field

    // Email validation
    QRegExp emailRx("^[\\w-\\.]+@[\\w-]+\\.[a-z]{2,4}$");
    QRegExpValidator* emailValidator = new QRegExpValidator(emailRx, this);
    ui->lineEdit_4->setValidator(emailValidator); // Assuming this is for the 'email' field

    // Telephone number validation (digits only)
    QRegExp telRx("^[0-9]+$");
    QRegExpValidator* telValidator = new QRegExpValidator(telRx, this);
    ui->lineEdit_5->setValidator(telValidator); // Assuming this is for the 'telephone' field
    ui->lineEdit_6->setValidator(telValidator); // Assuming this is for the 'fax' field
...
void WindowEntreprises::on_pushButton_clicked()
{
    ...
    // Email validation
    QRegExp emailRx("^[\\w-\\.]+@[\\w-]+\\.[a-z]{2,4}$");
    if (!emailRx.exactMatch(mail)) {
        QMessageBox::warning(this, "Invalid Email", "Please enter a valid email address.");
        return; // Stop further processing if the email is invalid
    }

    // If all validations pass, proceed to add the entreprise
